...
const mock_page_date_1 = require("./mock-page-date");
const { screenshotAndUploadToImgur, } = require('./screenshot-and-upload-to-imgur');
...
(async () => {
    ...
    if (argv.date) {
        await (0, mock_page_date_1.mockPageDate)(page, new Date(argv.date).getTime());
    }
    ...
    if (argv.useLocalScript) {
        const scriptCode = await fs.readFile(path.resolve(__dirname, '../build/index.js'), { encoding: 'utf-8' });
    ...
    const { wordleCount, shareText, guessesText } = await page.evaluate((solved) => {
        ...
        gameState.guesses.forEach(function (guess, index) {
            ...
            guessesTextLines.push(`${guessCount}. ${guessWord}${correctGuessIndicator}`);
        });
        return {
            wordleCount,
    ...
    if (!argv.skipImageUpload) {
        try {
            const imgUrl = await screenshotAndUploadToImgur(page, `Wordle ${wordleCount}`, guessesText);
